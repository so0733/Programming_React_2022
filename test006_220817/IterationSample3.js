...
const IterationSample3 = () => {
    ...
    const onRemove = id => {    // 데이터 제거 기능 구현
        const nextNames = names.filter(name => name.id !== id);
        setNames(nextNames);
    };
    const nameList = names.map(name => <li key={name.id} onDoubleClick={() => onRemove(name.id)}>{name.text}</li>)    // map 함수를 사용할 때 key 값을 index 대신 name.id값으로 지정함
    return (
        <>  
